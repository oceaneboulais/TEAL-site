...
addpath([code_path filesep 'functions'], [code_path filesep 'EnumClass']);
...
keyword='R3D_June2024';
TiDAL_ID=3;
base_dir0=load_directory_locations(keyword,TiDAL_ID);


subdirs=dir([base_dir0 filesep '2*']);  %Subdirectiory starts with a year
subdirs=subdirs([subdirs.isdir]);

for Idir=1:length(subdirs)

    base_dir=[base_dir0 filesep subdirs(Idir).name];
    Digital_fnames=dir([base_dir filesep 'DigitalData*.vs']);
    Analog_fnames=dir([base_dir filesep 'AnalogData*.vs']);

    eval(sprintf('!mkdir %s',subdirs(Idir).name));
    ...
    for Ifile=1:length(Analog_fnames)
        ...
        try
            ...
            if ~flag_digital_only
                audiowrite([subdirs(Idir).name filesep stem '.wav'],MeasuredValues,Fs,'BitsPerSample',32);
            end
            save([subdirs(Idir).name filesep strrep(stem,'Analog','Digital') '.mat'],'DigitalDataMeas','DigitalPar');
