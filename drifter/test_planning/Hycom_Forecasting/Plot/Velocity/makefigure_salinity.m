...
% Edited to make a salinity plot
...
switch zoomoption
    case 1
        figname = 'HYCOM_forecast_salinity_200';
        xl = [-127 -116];
        yl = [26.5 35];       
    case 2
        figname = 'HYCOM_forecast_salinity_200_zoom';
        xl = [-125 -121];
        yl = [30 34];
end
...
f = figure('visible','off');
  ...
  for movietime = 1:length(timevec)
    ...
    [~,t]=closest(tvec,targettime);
       ...
       [~,I_salt]=closest(data.depth,100);
       salinity = squeeze(data.salinity(lonI(1):lonI(end),latI(1):latI(end),I_salt,t));
      
        % Contour plot of salinity
       
        speedplot = imagescnan(longitude,latitude,salinity',[33.3 34.0]); 
        speedplot.HandleVisibility = 'off';
        axis xy;
        cmocean('balance',100);
        c = colorbar;
        c.Label.String = 'Salinity at 100m [PSU]';
        ...
        title(['<0-200m> Velocities at ' timestr ' with Salinity at 100m']);
