...
% elev_deg=asind(linspace(-1,1,1+(180/el_res_bf)));
% elev_deg=0.125*round(elev_deg/0.125);
elev_deg = -90:el_res_bf:90;
...
fs0 = 102400; % original sample rate 
fs_resample = 51200;
...
if ~isempty(dateset_utc)
    isintime = driftlog.DeployTimeUTC <= dateset_utc(1) & driftlog.RecoverTimeUTC >= dateset_utc(2);
...
for deployment = deployment_set
    ...
    if ~isempty(dateset_utc)
        t0_utc = max(driftlog.DeployTimeUTC(dive_index(1)),dateset_utc(1));
        tend_utc = min(driftlog.RecoverTimeUTC(dive_index(1)),dateset_utc(2));
    else
    ...
    savefolder = fullfile(procdata_basedir,mfilename,...
        sprintf('%s_fs%1.0f_nfft%1.0f_noverlp%1.0f_dEL%i_elev',array_type,fs,nfft,noverlp,el_res_bf),...
    ...
    for di = dive_index
        ...
        if ~isempty(dateset_utc)
            dive_time_start_utc = max(dive_time_start_utc,dateset_utc(1));
            dive_time_stop_utc = min(dive_time_stop_utc,dateset_utc(2));
        end
        ...
        for file_idx = 1:Nfiles
            ...
            if (do_bf_processing&&((~bffile_exists||overwrite_beam_data))||(do_vs_processing&&(~vsfile_exists||overwrite_avs_data)))
                ...
                try 
                    ...
                    if fs00~=fs0
                        warning('Invalid sample rate, skipping fiel %s',filelist(file_idx).name)
                        continue
                    end
                catch
...
%                 t_sec0 = convertTo(t_utc,'posixtime'); %t_sec = t_sec - t_sec(1);
            ...
            if do_vs_processing&&(~vsfile_exists||overwrite_avs_data)
                ...
                dyear = decyear(time_start_utc);
                try
                    [~, ~, declination] = wrldmagm(0, dlat, dlon, dyear);
                catch
                    warning('wrldmagm failed, attempting to read declination from deployment log')
                    declination = driftlog.MagneticDeclination(di);
                end
            ...
            if do_stats
                if do_bf_processing&&(overwrite_stats||~exist(bf_hist_filename,'file'))
                    ...
                    if bffile_exists&&~overwrite_beam_data
                        ...
                        B_pow = 10.^(double(beamdata.B_pow_dB)/(10*beamdata.kscale));
                    end
...
%                     computeVSHistogramMatrixND(avsdata,hist_param,tabs_files,file_idx);
                    % NOTE: this changes the global variable avs_hist !!
		...
		ax = [];
		    ...
		    if ~isempty(driftcam)
                    ...
                    linkaxes(ax,'x');
                    xlim(ax(1),[min(file_time_utc) max(file_time_utc)])
		        ...
		        if ~isempty(driftcam)
                    ...
                    if save_to_ppt
                        ...
                        pptx.addPicture(gcf);
                    elseif do_histogram_plots
                        png_savename = fullfile(sprintf('%1.2fkHz_to_%1.2fkHz_avs_az_hist_%s_%s',f1/1e3,f2/1e3,t_label{1},t_label{2}));
                        saveas(gcf,fullfile(savefolder,[png_savename '.png']))
                        saveas(gcf,fullfile(savefolder,[png_savename '.fig']))
                    end
                    set(groot,'CurrentFigure',avs_az_hist_fig); clf;
		        ...
		        if ~isempty(driftcam)
                    ...
                    if save_to_ppt
                        ...
                        pptx.addPicture(gcf);
                    elseif do_histogram_plots
                        png_savename = fullfile(sprintf('%1.2fkHz_to_%1.2fkHz_avs_el_hist_%s_%s',f1/1e3,f2/1e3,t_label{1},t_label{2}));
                        saveas(gcf,fullfile(savefolder,[png_savename '.png']))
                        saveas(gcf,fullfile(savefolder,[png_savename '.fig']))
                    end
                    set(groot,'CurrentFigure',avs_el_hist_fig); clf;
